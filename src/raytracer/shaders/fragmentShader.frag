...
// takes a point and the bounding points of a box,
// returns whether or not that point is inside the box
bool inBox(vec3 p, vec3 pmin, vec3 pmax) {
  if (p[0] + EPS >= pmin[0] - EPS && p[0]-EPS <= pmax[0] +EPS) {
    if (p[1] +EPS >= pmin[1] -EPS && p[1]-EPS <= pmax[1] +EPS) {
      if (p[2]+EPS >= pmin[2] -EPS && p[2]-EPS <= pmax[2]+EPS) {
        return true;
      }
    }
  }
...
vec3 sampleSphere(float x1, float x2) {
  vec3 result = vec3(0,0,0);
  result[0] = 2.0 * x1 * sqrt(1.0 - x1 * x1 - x2 * x2);
  result[1] = 2.0 * x2 * sqrt(1.0 - x1 * x1 - x2 * x2);
  result[2] = 1.0 - 2.0 * (x1 * x1 + x2 * x2);
  return result;
}
...
float findIntersectionWithTriangle(Ray ray, vec3 t1, vec3 t2, vec3 t3,
  ...
  // find normal of the plane defined by the 3 points
  vec3 dir = cross(t2 - t1, t3 - t1);
  vec3 norm = dir / length(dir);
  vec3 origin = ray.origin;
  float d = dot(norm, t1);

  // find position on plane that the ray intersects
  float dist = findIntersectionWithPlane(ray, norm, d, intersect);


  vec3 v1, v2, n1;
  vec3 p = intersect.position;
  bool isInsideObj = true;
...
float findIntersectionWithSphere(Ray ray, vec3 center, float radius,
  ...
  // find nearest closest intersection
  float t1 = tca - thc;
  float t2 = tca + thc;
  float t;
  if (t1 > EPS) {
  ...
  // calculate normal vector at intersection for lighting calculation
...
float findIntersectionWithBox(Ray ray, vec3 pmin, vec3 pmax,
  ...
  float d = dot(norm, pmax); 
  float dist = findIntersectionWithPlane(ray, norm, d, new_intersect);
  if (inBox(new_intersect.position, pmin, pmax)) {
    best_intersect.position = new_intersect.position;  
    best_intersect.normal = new_intersect.normal;
    best_dist = dist;
  }
  ...
  norm = vec3(-1,0,0);
  d = dot(norm, pmin); 
  dist = findIntersectionWithPlane(ray, norm, d, new_intersect);
  if (inBox(new_intersect.position, pmin, pmax)) {
    chooseCloserIntersection(dist, best_dist, new_intersect, best_intersect);
  }
...
float getIntersectOpenCylinder(Ray ray, vec3 center, vec3 axis, float len,
  ...
  float sigma = dot(vd, axis);
  float theta = dot(ray.direction, axis);
  float a = pow(length(ray.direction - theta * axis), 2.0);
  float b = 2.0 * dot(ray.direction - theta * axis, vd - sigma * axis);
  float c = pow(length(vd - sigma * axis), 2.0) - pow(rad, 2.0);
  ...
  float t;
  vec3 q1 = ray.origin + ray.direction * t1;
  vec3 q2 = ray.origin + ray.direction * t2;
  vec3 p2 = center + len;
  if (t1 <= 0.0) {
    vec3 q = ray.origin + (t2 * ray.direction);
    if (dot(axis, q - center) > -EPS && dot(axis, q - p2) < EPS) {
      t = t2;
    }
  }
  else if (t2 <=0.0) {
    vec3 q = ray.origin + (t1 * ray.direction);
    if (dot(axis, q - center) > -EPS && dot(axis, q - p2) < EPS) {
      t = t1;
    }
  }
  else {
    vec3 q1 = ray.origin + (t1 * ray.direction);
    vec3 q2 = ray.origin + (t2 * ray.direction);
    if (dot(axis, q1 - center) > -EPS && dot(axis, q1 - p2) < EPS) {
      if (dot(axis, q2 - center) > -EPS && dot(axis, q2 - p2) < EPS) {
        if (t1 >= t2) {
          t = t2;
        }
        else {
    ...
    else if (dot(axis, q2 - center) > -EPS && dot(axis, q2 - p2) < EPS) {
      t = t2;
    }
    else {
      return INFINITY;
...
float getIntersectDisc(Ray ray, vec3 center, vec3 norm, float rad,
  ...
  // ----------- Our reference solution uses 18 lines of code.
    ...
    float dist = findIntersectionWithPlane(ray, norm, d, intersect);
    if (dot(norm, intersect.position - center) < EPS) {
      if (pow(length(intersect.position - center), 2.0) - EPS < pow(rad, 2.0) + EPS) {
        return dist;
      }
    }
...
float getIntersectOpenCone(Ray ray, vec3 apex, vec3 axis, float len,
  ...
  float a = pow(length(ray.direction - theta * axis), 2.0) * pow(cos(alpha), 2.0) - pow(theta, 2.0) * pow(sin(alpha), 2.0);
  float b = 2.0 * (dot(ray.direction - theta * axis, vd - phi * axis) * pow(cos(alpha), 2.0) - theta * phi * pow(sin(alpha), 2.0));
  float c = pow(length(vd - phi * axis), 2.0) * pow(cos(alpha), 2.0) - pow(phi, 2.0) * pow(sin(alpha), 2.0);
  float discriminant = pow(b, 2.0) - 4.0 * a * c;
  if (discriminant < 0.0) {
    return INFINITY;
  }
  ...
  if (t1 < EPS) {
    vec3 q = ray.origin + (t2 * ray.direction);
    if (dot(axis, q - apex) > -EPS && dot(axis, q - p2) < EPS) {
      t = t2;
    }
  }
  else if (t2 < EPS) {
    ...
    vec3 q1 = ray.origin + (t1 * ray.direction);
    vec3 q2 = ray.origin + (t2 * ray.direction);
    if (dot(axis, q1 - apex) > -EPS && dot(axis, q1 - p2) < EPS) {
      if (dot(axis, q2 - apex) > -EPS && dot(axis, q2 - p2) < EPS) {
        if (t1 > t2) {
          t = t2;
        }
        else {
    ...
    else if (dot(axis, q2 - apex) > -EPS && dot(axis, q2 - p2) < EPS) {
      t = t2;
    }
    else {
      return INFINITY;
  ...
  // calculate normal vector at intersection for lighting calculation
  vec3 e = p - apex;
  vec3 norm = normalize(e - length(e) / cos(alpha) * normalize(axis));
  float d = dot(norm, p); 
  float dist = findIntersectionWithPlane(ray, norm, d, intersect);
  return dist;
  // ----------- STUDENT CODE END ------------
...
vec3 calculateSpecialDiffuseColor(Material mat, vec3 posIntersection,
  ...
  if (mat.special == CHECKERBOARD) {
    ...
    posIntersection += EPS;
    float sum = ceil(posIntersection[0]/8.0) + ceil(posIntersection[1]/8.0) + ceil(posIntersection[2]/8.0);
    if (mod(sum, 2.0) < EPS) {
      return vec3(0,0,0);
    }
...
float softShadowRatio(vec3 pos, vec3 lightVec) {
  ...
  float count = 0.0;
  float n = 3.0;
  float offset_x, offset_y, rand_x, rand_y, jitter_x, jitter_y, x1, x2;
  vec2 seed = vec2(0,0);
  vec3 random_offset = vec3(0,0,0);
  vec3 result = vec3(0,0,0);
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      offset_x = float(i) / n;
      ...
      jitter_y = offset_y + (rand_y / n);
      x1 = jitter_x * 2.0 - 1.0;
      x2 = jitter_y * 2.0 - 1.0;
      random_offset = sampleSphere(x1, x2);
      result = lightVec + random_offset;
      if (!pointInShadow(pos, result)) {
        count += 1.0;
      }
    }
  }
...
vec3 getLightContribution(Light light, Material mat, vec3 posIntersection,
  ...
  if (mat.materialType == PHONGMATERIAL ||
    ...
    if (mat.materialType == PHONGMATERIAL) {
      ...
      vec3 r = 2.0 * dot(lightVector, normalVector) * normalVector - lightVector;
      phongTerm = pow(dot(eyeVector, r), mat.shininess) * mat.specular * light.intensity / attenuation;
      // ----------- STUDENT CODE END ------------
...
vec3 calcReflectionVector(Material material, vec3 direction, vec3 normalVector,
  ...
  float angle_i = dot(normalize(normalVector), normalize(direction));
  float angle_r = sqrt(1.0 - (eta * eta) * (1.0 - (angle_i * angle_i)));
  vec3 result = (-eta * angle_i - angle_r) * normalVector + eta * direction;
  return result;
  // ----------- Our reference solution uses 5 lines of code.
...
vec3 traceRay(Ray ray) {
  ...
  for (int depth = 0; depth < MAX_RECURSION; depth++) {
    ...
    if (hitMaterial.reflectivity > EPS) {
      vec3 direction = normalize(calcReflectionVector(hitMaterial, ray.direction, normalVector, isInsideObj));
